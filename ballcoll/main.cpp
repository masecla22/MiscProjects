...
const int breadcrumbActive=1;
const int activateCollision=1;
const int showCircle =0;
struct points
{
    int x,y;
};
...
{
    for(int i=0; i<lenght-1; i++)a[i]=a[i+1];
    a[lenght-1].x=xPos;
    a[lenght-1].y=yPos;
}
...
struct ball
{
    ...
    int update (int whatUpdate)
    {
        ...
        hitBox.x=xPos;
        hitBox.y=yPos;
        if(xPos>=WIDTH || xPos<=0)xVel*=-1;
        if(yPos>=HEIGHT || yPos<=0)yVel*=-1;
        if(whatUpdate%breadCrumbPrec==0&& breadcrumbActive)if(whatPos<breadcrumbLenght)
                breadcrumbs[whatPos++]=genPoint(xPos,yPos);
            else pushArray(breadcrumbs,xPos,yPos,whatPos);
    }
    points getr()
    {
        points f;
    ...
    {
        if(hitBoxOn==1)hitBox.draw();
        if(showCircle==1)ellipse(xPos,yPos,0,0,20,20);
        if(breadcrumbActive==1)
            for(int i=0; i<whatPos-1; i++)line(breadcrumbs[i].x,breadcrumbs[i].y,breadcrumbs[i+1].x,breadcrumbs[i+1].y);
    }
    void reverses()
    {
...
int checkColl(ball the[200],int which,int howMany)
{
    for(int i=0; i<howMany; i++)
        if(i!=which)if(doOverlap(the[which].hitBox,the[i].hitBox)!=0)return i;
    return -1;
}
...
{
    for(int i=0; i<howMany; i++)the[i].draws();
}
int main()
{
    int i=0;
    ...
    while(true)
    {
        ...
        if(i%trailLenght==0)
            cleardevice();
        i++;
        if(GetAsyncKeyState(VK_DOWN)!=0 && (sleepVar-10)>0)sleepVar-=10;
        if(GetAsyncKeyState(VK_UP)!=0)sleepVar+=10;
    }
    getch();
    return 0;
}
